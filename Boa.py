...
def processArgs(argv):
    ...
    for opt, arg in optlist:
        ...
        if opt in ('-h', '--help', '-H', '--Help'):
            ...
            print(
                'Command-line usage: %s [options] [file1] [file2] ...' %
                main_script)
            print('-C, --Constricted:')
...
if __version__.wx_version_max and wxVersion > __version__.wx_version_max:
    ...
    wx.MessageBox('Sorry! This version of Boa does not work under '
                  ...
                  'wxPython %d.%d.%d.%d' % (
                      wxVersion + __version__.wx_version_max),
                  'Version error', wx.OK | wx.ICON_ERROR)
...
class BoaApp(wx.App):
    ...
    def OnInit(self):
        ...
        wx.Locale.AddCatalogLookupPathPrefix(
            os.path.join(Preferences.pyPath, 'locale'))
        if hasattr(sys, 'frozen'):
    ...
    # editor.setupToolBar()
        ...
        if wx.Platform == '__WXMSW__':
            self.tbicon = wxadv.TaskBarIcon()
            self.tbicon.SetIcon(self.main.GetIcon(), 'Boa Constructor')
            self.tbicon.Bind(
                wxadv.EVT_TASKBAR_LEFT_DCLICK,
                self.OnTaskBarActivate)
            self.tbicon.Bind(wxadv.EVT_TASKBAR_RIGHT_UP, self.OnTaskBarMenu)
            self.tbicon.Bind(
                wx.EVT_MENU,
                self.OnTaskBarActivate,
