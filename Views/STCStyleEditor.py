...
class STCStyleEditDlg(wx.Dialog):
    ...
    def _init_ctrls(self, prnt):
        ...
        self.speedsettingCh.SetHelpText(
            _('The speed setting allows you to revert to one of the predefined style sets. This will overwrite your current settings when tha dialog is posted.'))
        ...
        self.elementLb.SetHelpText(
            _('Select a style here to edit it. Common definitions can be added and maintained here.  A common definition is a property that can be shared between styles and special cased per platform.'))
        ...
        self.removeCommonItemBtn.SetToolTipString(
            _('Remove the selected Common definition'))
        ...
        self.stc.SetHelpText(
            _('The style preview window. Click or move the cursor over a specific style to select the style for editing in the editors above.'))
        ...
        self.cancelBtn.SetConstraints(LayoutAnchors(self.cancelBtn, False,
                                                    False, True, True))
        self.cancelBtn.SetToolTipString(
            _('Close dialog without saving changes'))
        ...
        self.fixedWidthChk.SetToolTipString(
            _('Check this for Fixed Width fonts'))
    ...
    def __init__(self, parent, langTitle, lang, configFile, STCsToUpdate=()):
        ...
        for combo, okBtn, evtRet, evtCB, evtRDC in (
            (self.fgColCb, self.fgColOkBtn, self.OnfgColRet,
             self.OnfgColCombobox, self.OnGotoCommonDef),
            (self.bgColCb, self.bgColOkBtn, self.OnbgColRet,
                self.OnbgColCombobox, self.OnGotoCommonDef),
            (self.faceCb, self.faceOkBtn, self.OnfaceRet,
                self.OnfaceCombobox, self.OnGotoCommonDef),
...
# ---Control population methods-------------------------------------------
    ...
    def bindComboEvts(self, combo, btn, btnEvtMeth,
                      comboEvtMeth, rdclickEvtMeth):
    ...
    def populateCombosWithCommonDefs(self, fixedWidthOnly=None):
        ...
        try:
            ...
            fontNameList = sorted(fontEnum.GetFacenames())
    ...
    def populateStyleSelector(self):
        numStyles = sorted(self.styleIdNames.items())
        self.styleNumLookup = {}
        stdStart = -1
        stdOffset = 0
        extrOffset = 0
        ...
        for num, name in numStyles:
            if num == wx.stc.STC_STYLE_DEFAULT:
                ...
                # extra styles
                    ...
                    self.elementLb.InsertItems(
                        [name], stdStart + extrOffset - 1)
        ...
        try:
            if dlg.ShowModal() == wx.ID_OK:
                ...
                if name not in self.commonDefs:
                    ...
                    self.elementLb.SetSelection(
                        self.elementLb.GetCount() - 1, True)
                    self.populateCombosWithCommonDefs()
                    self.OnElementlbListbox(None)
        finally:
            dlg.Destroy()
...
# ---STC events-----------------------------------------------------------
    def OnUpdateUI(self, event):
        ...
        if styleBefore in self.styleIdNames:
            ...
            self.styleDefST.SetLabel(
                _('Style %i not defined, sorry.') %
                styleBefore)
        self.OnElementlbListbox(None)
        event.Skip()
...
def setSTCStyles(stc, styles, styleIdNames, commonDefs, lang, lexer, keywords):
    ...
    newStyles = []
    styleItems = sorted(list(styleIdNames.items()) +
                        list(settingsIdNames.items()))
    idx = 0
