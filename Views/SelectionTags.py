...
class SelectionGroup:
    ...
    def moveRelease(self):
        ...
        if self.dragging and self.selection:
            ...
            wx.PostEvent(
                self.selection, wx.SizeEvent(
                    self.selection.GetSize()))
    ...
    def setSelection(self, finishDragging=False):
        ...
        if not self.dragging:
            ...
            self.stT.SetDimensions(
                trPos.x - offsetO + int(round(trSze.x / 2.0)), trPos.y - offsetO, tagSize, tagSize)
            self.stB.SetDimensions(trPos.x -
                                   offsetO +
                                   int(round(trSze.x /
                                             2.0)), trPos.y -
                                   offsetI +
                                   trSze.y, tagSize, tagSize)
            self.stL.SetDimensions(trPos.x - offsetO,
                                   trPos.y - offsetO +
                                   int(round(trSze.y / 2.0)),
                                   tagSize,
                                   tagSize)
            self.stR.SetDimensions(trPos.x -
                                   offsetI +
                                   trSze.x, trPos.y -
                                   offsetO +
                                   int(round(trSze.y /
        ...
        self.slT.SetDimensions(
            trPos.x - frmWid,
            trPos.y - frmWid,
            trSze.x + frmWid,
            frmWid)
        self.slR.SetDimensions(
            trPos.x + trSze.x,
            trPos.y - frmWid,
            frmWid,
            trSze.y + frmWid * 2)
        self.slB.SetDimensions(
            trPos.x - frmWid,
            trPos.y + trSze.y,
            trSze.x + frmWid * 2,
            frmWid)
        self.slL.SetDimensions(
            trPos.x - frmWid,
            trPos.y - frmWid,
            frmWid,
            trSze.y + frmWid)
        ...
        if self.selCompn:
            for idx in range(4):
                if self.selCompn.anchorSettings:
                    self.anchorTags[idx].setAnchor(
                        self.selCompn.anchorSettings[idx])
                else:
...
class SingleSelectionGroup(SelectionGroup):
    ...
    def positionUpdate(self):
        if self.selCompn:
            self.updateInspectorPageProps(
                self.selCompn.getPositionDependentProps())

    def sizeUpdate(self):
        if self.selCompn:
            self.updateInspectorPageProps(
                self.selCompn.getSizeDependentProps())
...
class SelectionTag(wx.Panel):
    ...
    def __init__(self, parent, cursor, tagSize, group,
                 pnlStyle):  # = wx.SIMPLE_BORDER):
        wx.Panel.__init__(self, parent, -
                          1, size=wx.Size(tagSize, tagSize), style=pnlStyle)
        self.Hide()
        self.group = group
    ...
    def setPos(self, position, mirrorX, mirrorY,
               mirrorXY, oldPos=None, oldSize=None):
        grp = self.group
...
def openCollEditorForSizerItems(
        inspector, companion, sizersView=None, sizer=None):
    if sizersView is None:
        sizersView = companion.designer.sizersView
