...
class DiffPSOut(Utils.PseudoFile):
    def write(self, s):
        ...
        if s not in ('+', '-', ' ', '?'):
            ...
            self.output.MarkerAdd(
                self.output.GetLineCount() - 1,
                uniqueFile2Mrk)
        elif s[0] == '-':
            self.output.MarkerAdd(
                self.output.GetLineCount() - 1,
...
class PythonSourceDiffView(wx.stc.StyledTextCtrl, EditorView,
    ...
    def __init__(self, parent, model):
        ...
        EditorView.__init__(self, model,
                            ...
                            (('-', None, '', ''),
                             (_('Previous difference'),
                              self.OnPrev, self.prevBmp, ''),
                                (_('Next difference'),
                                 self.OnNext, self.nextBmp, ''),
        ...
        wx.stc.EVT_STC_MARGINCLICK(
            self, wxID_PYTHONSOURCEDIFFVIEW, self.OnMarginClick)
        ...
        if self.diffWith:
            ...
            try:
                ...
                try:
                    ...
                    dst = StringIO(
                        openEx(
                            self.diffWith).load('rb')).readlines()
