...
class CollectionEditor(wx.Frame, Utils.FrameRestorerMixin):
    def _init_ctrls(self, prnt):
        ...
        self.toolBar = wx.ToolBar(size=wx.DefaultSize,
                                  id=wxID_COLLECTIONEDITORTOOLBAR, pos=wx.Point(
                                      32, 4),
...
class ImageListCollectionEditor(CollectionEditor):
    def __init__(self, parent, collEditView, additMeths=()):
        CollectionEditor.__init__(
            self,
            parent,
            collEditView,
            additMeths,
            wx.LC_REPORT)
        self.itemList.SetImageList(
            collEditView.companion.control,
            wx.IMAGE_LIST_SMALL)
    ...
    def selectNone(self):
        if self.frame:
            for itemIdx in range(self.frame.itemList.GetItemCount()):
                ...
                self.frame.itemList.SetItemState(
                    itemIdx, 0, wx.LIST_STATE_SELECTED)
...
if __name__ == '__main__':
    ...
    frame = CollectionEditor(
        None, cev, ('New item', '-', 'Append separator', 'New sub-menu'))
    frame.Show(True)
    app.MainLoop()
