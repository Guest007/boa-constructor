...
class PersistentOGLView(ScrollingContainer, EditorViews.EditorView):
    ...
    def __init__(self, parent, model, actions=()):
        ScrollingContainer.__init__(
            self, parent, wx.Size(
                self.defSize, self.defSize))
        EditorViews.EditorView.__init__(self, model,
                                        ((_('(Re)load diagram'), self.OnLoad, self.loadBmp, ''),
                                         (_('Save diagram'),
                                          self.OnSave, self.saveBmp, ''),
                                            (_('Print diagram to PostScript'),
                                             self.OnPrintToPS, '-', ''),
                                            ('-', None, '-', ''),
                                            (_('Change size'),
                                             self.OnSetSize, '-', ''),
...
# self.diagram.Destroy()
    ...
    def newLine(self, dc, fromShape, toShape):
        ...
        # for some reason, the shapes have to be moved for the line to show
        # up...
...
#        shape.SetFont(wx.Font(6, wx.MODERN, wx.NORMAL, wx.NORMAL, False))
    ...
    def OnLoad(self, event):
        self.canvas.loadSizes(
            os.path.splitext(
                self.model.filename)[0] +
            self.ext)

    def OnSave(self, event):
        self.canvas.saveSizes(
            os.path.splitext(
                self.model.filename)[0] +
            self.ext)

    def OnPrintToPS(self, event):
        self.canvas.printSizes(
            os.path.splitext(
                self.model.filename)[0] + '.ps')
...
class SortedUMLViewMix:
    ...
    def buildShapes(self, dc):
        ...
        for parent, child in routes:
            self.newLine(dc, self.getCurrentShape(child),
    ...
    def arrangeShapes(self, dc, nodes, routes, whiteSpaceFactor=1.1):
        ...
        verticalWhiteSpace = (height - rawHeight) / \
            (len(generations) - 1.0 or 2.0)
        # fudge factors to keep some extra space
        self.setSize(wx.Size(int(width + 50), int(height + 50)))
        # distribute each generation across the width
        # and the generations across height
        y = 0
        for currentWidth, currentHeight, generation in map(
                None, widths, heights, generations):
            x = 0
            # whiteSpace is the space between any two elements...
            whiteSpace = (width - currentWidth) / \
            ...
            for className in generation:
                ...
                csX, csY = self.diagram.Snap(
                    (shapeX / 2.0) + x, (shapeY / 2.0) + y)
...
class UMLView(PersistentOGLView, SortedUMLViewMix):
    ...
    def newClass(self, size, pos, className, classMeths, classAttrs):
        ...
        regionAttribs.SetProportions(
            0.0, 1.0 * (attribsHeight / float(totHeight)))
...
class ImportsView(PersistentOGLView):
    ...
    def refreshCtrl(self):
        ...
        for rel in list(relations.keys()):
            ...
            shape, width = self.newModule(
                (20, 30), (p, y), rel, list(
                    relations[rel].classes.keys()))
