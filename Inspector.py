...
from Preferences import IS, flatTools, inspPageNames, keyDefs, oiLineHeight
...
[wxID_INSPECTORFRAMETOOLBARTOOLS0] = [wx.NewId()
                                      for _init_coll_toolBar_Tools in range(1)]
    ...
    def __init__(self, parent):
        ...
        for cmpInf in list(PaletteStore.compInfo.values()):
            ...
            try:
                ...
                cmpInf.append(
                    self.paletteImages.Add(
                        IS.load('Images/Palette/Component.png')))
...
# ---Object selection-----------------------------------------------------
    def selectObject(self, compn, selectInContainment=True, collDesgn=None,
        ...
        if self.prevDesigner[0] and self.prevDesigner != (
                compn.designer, collDesgn):
    ...
    def directPositionUpdate(self, comp):
        comp.persistProp(
            'Position', 'SetPosition', repr(
                comp.control.GetPosition()))
...
# else: return ''
    ...
    def OnDelItem(self, event):
        if self.selCmp and self.props.prevSel and hasattr(
                self.selCmp, 'propItems'):
...
class ParentTree(wx.TreeCtrl):
    ...
    def addChildren(self, parent, dict, designer):
        ...
        for par in designer.objectOrder:
            if par in dict:
                img = PaletteStore.compInfo[designer.objects[par]
                                            [1].__class__][2]
        ...
        if self.valid:
            ...
            if self.designer:
                ...
                if hasattr(self.designer,
                           'selection') and self.designer.selection:
                    self.designer.selection.selectCtrl(
                        ctrlInfo[1], ctrlInfo[0])
...
class NameValue:
    ...
    def __init__(self, inspector, nameParent, valueParent, companion,
        ...
        if self.propEditor:
            ...
            if PropertyEditors.esExpandable in self.propEditor.getStyle():
                ...
                self.expander = wx.CheckBox(nameParent, mID, '',
                                            wx.Point(
                                                8 * self.indent, self.idx * oiLineHeight + 2),
                                            wx.Size(13, 14))
        ...
        self.nameCtrl = wx.lib.stattext.GenStaticText(nameParent, -1, name,
                                                      wx.Point(
                                                          8 * self.indent + 16, idx * oiLineHeight + 2),
                                                      wx.Size(
                                                          inspector.panelNames.GetSize().x, oiLineHeight - 3),
    ...
    def showPropNameModified(self, displayAsCat=False):
        if self.propEditor and Preferences.showModifiedProps:
            ...
            mod = not propEdit.companion.propIsDefault(
                propEdit.name, propSetter)
    ...
    def resize(self, nameWidth, valueWidth):
        if nameWidth != self.lastSizeN:
            ...
            if nameWidth > 100:
                self.nameCtrl.SetSize(
                    wx.Size(
                        nameWidth,
                        self.nameCtrl.GetSize().y))
            else:
    ...
    def showEdit(self):
        ...
        if not self.locked and self.propEditor:
            ...
            self.valueBevelTop = wx.Window(self.valueParent, -1,
                                           wx.Point(
                                               0, max(self.idx * oiLineHeight - 1, 0)),
    ...
    def hideEditor(self, cancel=False, noUpdate=False):
        if self.propEditor:  # and (not self.destr):
            if cancel:
                ...
                if noUpdate:
                    # swallow exceptions as autoposted values can not be
                    # canceled
                    try:
...
class EventsWindow(wx.SplitterWindow):
    ...
    def __init__(self, *_args, **_kwargs):
        ...
        self.categories = wx.SplitterWindow(self, -1,
                                            style=wx.SP_LIVE_UPDATE)  # wxNO_3D | wx.SP_3D | wx.SP_LIVE_UPDATE)
        ...
        self.categoryClasses = wx.ListCtrl(
            self.categories, wxID_EVTCATS, style=wx.LC_LIST)
        self.selCatClass = -1
        ...
        self.categoryMacros = wx.ListCtrl(
            self.categories, wxID_EVTMACS, style=wx.LC_LIST)
        ...
        self.categories.SplitVertically(
            self.categoryClasses, self.categoryMacros)
    ...
    def OnCatClassSelect(self, event):
        self.selCatClass = event.GetIndex()
    ...
    def OnMacClassSelect(self, event):
        self.selMacClass = event.GetIndex()
...
class NameValueEditorScrollWin(wx.ScrolledWindow):
    ...
    def refreshSplitter(self):
        ...
        if hOffset and len(self.nameValues) < s.y / hOffset:
            hOffset = 0
        self.splitter.SetDimensions(
            wOffset * puw, hOffset * puh * -1, s.x, s.y)
    ...
    def initFromComponent(self, name):
        ...
        if nv:
            ...
            for nv in self.getSubItems(nv):
                ...
                nv.propEditor.propWrapper.connect(
                    nv.propEditor.companion.obj, nv.propEditor.companion)
                nv.initFromComponent()
...
class InspectorScrollWin(NameValueEditorScrollWin):
    ...
    def __init__(self, parent, id=-1, pos=wx.DefaultPosition, size=wx.DefaultSize,
        ...
        self.SetAcceleratorTable(wx.AcceleratorTable([
            ...
            (keyDefs['ContextHelp'][0],
             keyDefs['ContextHelp'][1],
             wxID_CONTEXTHELP),
    ...
    def OnContextHelp(self, event):
        if self.inspector.selCmp:
            wxClass, url = self.extendHelpUrl(
                self.inspector.selCmp.GetClass(), '')
            if wxClass:
...
class InspectorPropScrollWin(InspectorScrollWin):
    ...
    def setNameValues(self, compn, rootCompn, nameValues,
                      insIdx, indent, ownerPropEdit=None):
        top = insIdx
    ...
    def expand(self, nameValue):
        ...
        obj = nv.propValue
        compn = nv.propEditor.getSubCompanion()(nameValue.propName, self.inspector.selCmp.designer,
...
class InspectorConstrScrollWin(InspectorScrollWin):
    ...
    def readObject(self, constrList):
        ...
        paramNames = sorted(
            list(
                params.keys()) +
            list(
                self.inspector.selCmp.extraConstrProps().keys()))
    ...
    def addParams(self, constrList, paramNames, compn,
                  rootCompn, indent=0, insIdx=-1):
    ...
    def addConstr(self, name, compn, rootCompn, indent=0, insIdx=-1):
        ...
        self.nameValues.insert(insIdx, ConstrNameValue(self, self.panelNames,
                                                       ...
                                                       insIdx, indent,
                                                       compn.getPropEditor(
                                                           name), compn.getPropOptions(name),
                                                       compn.getPropNames(name)))

    def addProp(self, name, compn, rootCompn, propWrap,
                indent=0, insIdx=-1, ownerPropEdit=None):
        if insIdx == -1:
            insIdx = len(self.nameValues)
...
class InspectorNotebook(wx.Notebook):
    ...
    def extendHelpUrl(self, cls):
        return self.pages[self.GetPageText(
            self.GetSelection())].extendHelpUrl(cls, '')
