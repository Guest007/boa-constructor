...
from . import Controllers, EditorHelper, EditorModels, PythonEditorModels
from .Controllers import EditorController, SourceController, addTool
...
class ModuleController(SourceController):
    ...
    def actions(self, model):
        actions = [
            ...
            (_('Check source'), self.OnCheckSource,
             self.compileBmp, 'CheckSource'),
    ...
    def createNewModel(self, modelParent=None):
        if modelParent:
            name = self.editor.getValidName(
                self.Model, modelParent.absModulesPaths())
        else:
    ...
    def OnProfile(self, event):
        ...
        if modtime is not None:
            ...
            if curmodtime == modtime:
                wx.LogError(
                    _('Stats file date unchanged, check for errors in script.'))
                return
            ...
            if dlg.ShowModal() == wx.ID_OK:
                ...
                if debugger and debugger.filename == model.localFilename():
                    if model.lastRunParams:
                        params = methodparse.safesplitfields(
                            model.lastRunParams, ' ')
                    else:
                        params = []
...
##                editor.statusBar.setHint('Finished execution.')
    ...
    def OnSwitchApp(self, event):
        model = self.getModel()
        if model and isinstance(
                model, PythonEditorModels.ModuleModel) and model.app:
            # does this ensure correct app is reconnected?
            appmodel, controller = self.editor.openOrGotoModule(
                model.app.filename)
    ...
    def OnConfigPyChecker(self, event):
        ...
        if model:
            ...
            SourceBasedPrefColNode('PyChecker', ('*',),
                                   appConfig, -1, None).open(self.editor)
    ...
    def OnCyclops(self, event):
        ...
        if model:
            ...
            resName = _('Cyclops report: %s') % time.strftime(
                '%H:%M:%S', time.localtime(time.time()))
            if resName not in model.views:
                resultView = self.editor.addNewView(
                    resName, EditorViews.CyclopsView)
            else:
                resultView = model.views[resName]
            resultView.tabName = resName
    ...
    def createNewModel(self, modelParent=None):
        ...
        if success:
            model = self.createModel(
                sourceconst.defPackageSrc, filename, '', False)
...
class PythonExtensionController(EditorController):
    ...
    def createNewModel(self, modelParent=None):
        raise Exception(
            _('Cannot create a new Python Extension, use distutils to build it'))
...
class SetupController(ModuleController):
    ...
    DefaultViews = ModuleController.DefaultViews + \
        [EditorViews.DistUtilManifestView]
    ...
    def actions(self, model):
        ...
        if wx.Platform == '__WXGTK__':
            actions.append(
                ('setup.py bdist_rpm', self.OnSetupBDist_RPM, '-', ''))
        else:
            actions.append(
                ('setup.py bdist_wininst', self.OnSetupBDist_WinInst, '-', ''))
...
def isPackage(filename):
    return os.path.exists(os.path.join(
        filename, PythonEditorModels.PackageModel.pckgIdnt))
