...
imageExtReg = []
# Dict of ext:Model entries. For types where not all files of that ext are
# images
imageSubTypeExtReg = {}
# List of extensions for internal filetypes created by Boa
internalFilesReg = []
# Dict of ext:Model entries which can be further identified by reading a
...
def initExtMap():
    ...
    for mod in list(modelReg.values()):
        if mod.ext not in ['.*', '.intfile', '.pybin'] + \
                list(inspectableFilesReg.keys()):
            extMap[mod.ext] = mod
