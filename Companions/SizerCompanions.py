...
from PropEdit.PropertyEditors import (BoolConstrPropEdit, BoolPropEdit,
                                      CollectionPropEdit, ConstrPropEdit,
                                      EnumConstrPropEdit, EnumPropEdit,
                                      FlagsConstrPropEdit, IntConstrPropEdit,
                                      IntPropEdit, ObjEnumConstrPropEdit,
                                      ReadOnlyConstrPropEdit,
                                      SizeConstrPropEdit,
                                      SizerEnumConstrPropEdit,
                                      StrConstrPropEdit, WinEnumConstrPropEdit,
                                      esExpandable)
from Utils import _

from . import Companions
from .BaseCompanions import (NYIDTC, CollectionDTC, CollectionIddDTC,
                             HelperDTC, UtilityDTC)
...
class SizerItemsCDTC(CollectionDTC):
    ...
    def designTimeDefaults(self, vals, method=None):
        ...
        if method in ('AddWindow', 'AddSizer', 'Insert'):
            if method in ('AddWindow', 'AddSizer'):
                ctrlIdx = 0
            elif method == 'Insert':
                ctrlIdx = 1
...
class FlexGridSizerDTC(GridSizerDTC):
    def __init__(self, name, designer, objClass):
        ...
        self.names['FlexibleDirection'] = {'wx.VERTICAL': wx.VERTICAL,
                                           'wx.HORIZONTAL': wx.HORIZONTAL,
                                           'wx.BOTH': wx.BOTH}
        self.options['FlexibleDirection'] = [
            wx.VERTICAL, wx.HORIZONTAL, wx.BOTH]
...
class ControlLinkedSizerDTC(SizerDTC):
    ...
    def designTimeObject(self, args=None):
        if args is not None:
            ...
            linkObjs = list(
                self.designer.controllerView.getObjectsOfClass(
                    self.LinkClass).items())
            if not linkObjs:
            ...
            try:
                if dlg.ShowModal() != wx.ID_OK:
                    raise Exception(
                        _('Must choose a control to link with sizer'))
...
class GBSizerItemsCDTC(SizerItemsCDTC):
    ...
    def applyDesignTimeDefaults(self, params, method=None):
        ...
        if self.control.FindItemAtPosition(pos):
            raise Exception(
                _('Cannot use Position %s, an item is already at that position') %
                params[1])
        if pos[0] < 0 or pos[1] < 0:
            raise Exception(
                _('Position %s invalid, cannot contain negative values') %
                params[1])

        span = self.eval(params['span'])
        if span[0] < 0 or span[1] < 0:
            raise Exception(
                _('Span %s invalid, cannot contain negative values') %
                params[1])
...
class RowColDTC(HelperDTC):
    ...
    def SetRowCol(self, name, value):
        ...
        # XXX Not sure how to handle this everywhere in the GridBagSizer
        # support
