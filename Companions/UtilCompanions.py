...
from PropEdit.PropertyEditors import (BitmapConstrPropEdit, BoolConstrPropEdit,
                                      BoolPropEdit, CollectionPropEdit,
                                      ColourConstrPropEdit, ConstrPropEdit,
                                      EnumConstrPropEdit, IntConstrPropEdit,
                                      LCCEdgeConstrPropEdit,
                                      MenuEnumConstrPropEdit,
                                      StrConstrPropEdit, WinEnumConstrPropEdit)
from Utils import _

from . import Constructors
from .BaseCompanions import NYIDTC, CollectionDTC, CollectionIddDTC, UtilityDTC
        ...
        if self.displayProp in tcl.params:
            ...
            if value != 'wx.NullBitmap':
                if value.startswith('wx.Bitmap'):
                    ...
                    if m:
                        value = '%s.%s' % (
                            m.group('imppath'), m.group('imgname'))
        return value

    def setBitmap(self, value):
        if value != wx.NullBitmap:
            self.control.Replace(self.index, value)
            self.designer.collEditors[(
                self.name, self.propName)].refreshCtrl(True)
...
# return ['%s%s = []'%(sourceconst.bodyIndent,
# self.__class__.sourceObjName), '']

    def finaliser(self):
        # , self.__class__.sourceObjName)]
        return ['', '%sreturn parent' % sourceconst.bodyIndent]
