...
class BoaDebugger (PropertyManager, SimpleItem):
    ...
    __ac_permissions__ = PropertyManager.__ac_permissions__ + (
        (view_management_screens, ('manage_start',
                                   'manage_stop', 'manage_callBreakpoint')),
    ) + SimpleItem.__ac_permissions__

    property_extensible_schema__ = 0
    _properties = PropertyManager._properties + (
        # why would you change this??? {'id':'host',     'type':'string',
        # 'mode':'w'},
    ...
    def manage_start(self, REQUEST=None):
        ...
        if getattr(self, '_v_active', False):
            REQUEST.set(
                'manage_tabs_message',
                'The debug server is already active')
        ...
        if REQUEST:
            REQUEST.set(
                'manage_tabs_message',
                'The debug server is active now')
            return self.manage_main(self, REQUEST)
    ...
    def manage_stop(self, REQUEST=None):
        ...
        if not getattr(self, '_v_active', False):
            if REQUEST:
                REQUEST.set(
                    'manage_tabs_message',
                    'The debug server is already stopped')
                return self.manage_main(self, REQUEST)
            else:
                return
        try:
        ...
        if REQUEST:
            REQUEST.set(
                'manage_tabs_message',
                'The debug server is stopped now')
            return self.manage_main(self, REQUEST)
    ...
    def manage_callBreakpoint(self, REQUEST=None):
        ...
        if REQUEST:
            REQUEST.set('manage_tabs_message',
                        '''The 'sys' module does not provide breakpoints.''')
            return self.manage_main(self, REQUEST)
        else:
