...
class KeyDefsDialog(wx.Dialog):
    ...
    def validateCtrls(self):
        ...
        if keyCode not in list(specialKeys.keys()) + otherKeys1 + otherKeys2:
            if len(keyCode) != 1 or keyCode not in string.letters + \
                    string.digits:
                raise InvalidValueError(
                    _('Key code must either be a single character (letter or digit) or an identifier selected from the combobox'))
            keyCode = "ord('%s')" % keyCode.upper()

        shortcut = self.shortcutTc.GetValue()
        if not shortcut:
            raise InvalidValueError(
                _('Shortcut may not be blank, enter a concise description, e.g. Ctrl-Shift-S'))
