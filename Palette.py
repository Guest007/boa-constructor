...
class BoaFrame(wx.Frame, Utils.FrameRestorerMixin):
    ...
    def initPalette(self, inspector, editor):
        ...
        if not hasattr(sys, 'cyclops'):
            ...
            if mb:
                mb.Append(
                    menu=palettePage.menu,
                    title=PaletteStore.newPalette[0])
...
class PanelPalettePage(wx.Panel, BasePalettePage):
    ...
    def __init__(self, parent, name, bitmapPath,
                 eventOwner, widgets, components, palette):
        ...
        self.posY = int(
            round((self.GetSize().y - (24 + self.buttonBorder)) / 2.0))
