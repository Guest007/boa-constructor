...
class FindReplaceDlg(wx.Dialog):
    ...
    def _init_ctrls(self, prnt):
        ...
        self.cmbFileFilter.SetToolTipString(
            _('Files that will be included in search'))
        ...
        self.directionRB = wx.RadioBox(choices=[_('Forward'), _('Backward')],
                                       id=wxID_FINDREPLACEDLGDIRECTIONRB, label=_(
                                           'Direction'),
        ...
        self.btnBuildInFind.SetToolTipString(
            _('Build-in ready to run searches'))
    ...
    def __init__(self, parent, engine, view, bModeFindReplace=1):
        ...
        self.findTxt.SetAcceleratorTable(wx.AcceleratorTable(
            [(wx.ACCEL_NORMAL, wx.WXK_RETURN, wxID_FINDRETURN)]))
        wxID_REPLACERETURN = wx.NewId()
        self.Bind(
            wx.EVT_MENU,
            self.OnReplacetxtTextEnter,
            id=wxID_REPLACERETURN)
        self.replaceTxt.SetAcceleratorTable(wx.AcceleratorTable(
            [(wx.ACCEL_NORMAL, wx.WXK_RETURN, wxID_FINDRETURN)]))
        ...
        self._mapBuildInFinds = {
            'Derived classes': r'\s*class\s+[a-zA-Z_][a-zA-Z0-9_]*\s*\(.*[^a-zA-Z0-9_]?XXXX[^a-zA-Z0-9_]?.*\)',
            'Class definition': r'\s*class\s+XXXX(\s|[^a-zA-Z0-9_])',
            r'Function\method definition': r'\s*def\s+XXXX(\s|[^a-zA-Z0-9_])',
            'Class member': r'(\s|[^a-zA-Z0-9_])self\.+XXXX(\s|[^a-zA-Z0-9_])'
        }
    ...
    def _validate(self, what):
        ...
        if what in ['find', 'findAll']:
            ...
            if not self.replaceTxt.GetValue():
                wx.MessageBox(
                    _('There is nothing to replace with.'),
                    _('Boa - Find/Replace'))
                return False
        elif what == 'findInFiles':
            if not os.path.isdir(self.cmbFolder.GetValue()):
                wx.MessageBox(
                    _('The folder you entered is not valid folder.'),
                    _('Boa - Find/Replace'))
                return False
        else:
            pass
    ...
    def _checkSelectionDlgOverlap(self):
        if self.view.viewName not in ('Package', 'Application'):
            ...
            if selStart != selEnd and selStartLnNo == self.view.LineFromPosition(
                    selEnd):
                chrHeight = self.view.GetCharHeight()
                selPos = self.view.ClientToScreen(
                    self.view.PointFromPosition(selStart))
                ...
                if r is not None:
                    ...
                    if selStartLnNo < self.view.GetFirstVisibleLine() + \
                        ...
                        self.SetPosition(
                            (dlgPos.x, selPos.y + chrHeight + self._fudgeOffset))
                    else:
                        self.SetPosition(
                            (dlgPos.x, selPos.y - dlgSize.y - self._fudgeOffset))
    ...
    def OnBuildInFind(self, event):
        dlg = wx.SingleChoiceDialog(
            self, _('Find'), _('Build in finds'), list(
                self._mapBuildInFinds.keys()))
        try:
...
'''
    ...
    def createFramedView(caption, View, model, parent=None,
                         size=wx.DefaultSize):
    ...
    class PseudoEditor:
        ...
        def addNewView(self, name, View): return createFramedView(
            name, View, self.model, self.parent, (400, 200))[1]
    ...
    f, v = createFramedView(
        'Find/Replace Dlg Tester (Cancel to quit)', PythonSourceView, m, size=(600, 400))
    v.refreshCtrl()
