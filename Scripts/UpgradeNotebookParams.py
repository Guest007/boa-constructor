...
addPageParams = re.compile("\s*parent\.AddPage\("
                           r"(?P<select>bSelect)\s*=\s*((true|false)|[\w\.]+),\s*"
                           r"(?P<imageId>imageId)\s*=\s*([-]?[0-9]+|[\w\.]+),\s*"
                           r"(?P<page>pPage)\s*=\s*self.\w+,\s*"
...
if __name__ == '__main__':
    assert len(
        sys.argv) >= 2, 'Missing filename(s) on the command-line to correct'
