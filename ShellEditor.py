...
class ShellEditor(wx.stc.StyledTextCtrl,
    ...
    def __init__(self, parent, wId):
        ...
        self.Bind(
            wx.EVT_MENU,
            self.OnShellCodeComplete,
            id=wxID_SHELL_CODECOMP)
    ...
    def bindShortcuts(self):
        ...
        self.sc = {}
        self.sc[(keyDefs['HistoryUp'][0],
                 keyDefs['HistoryUp'][1])] = self.OnHistoryUp
        self.sc[(keyDefs['HistoryDown'][0],
                 keyDefs['HistoryDown'][1])] = self.OnHistoryDown
        self.sc[(keyDefs['CodeComplete'][0], keyDefs['CodeComplete'][1])
                ] = self.OnShellCodeComplete
        self.sc[(keyDefs['CallTips'][0], keyDefs['CallTips'][1])
                ] = self.OnShellCallTips
    ...
    def execStartupScript(self, startupfile):
        if startupfile:
            ...
            self.pushLine(
                'print %s;execfile(%s)' %
                (repr(startuptext), repr(startupfile)))
        else:
            self.pushLine('')
    ...
    def debugShell(self, doDebug, debugger):
        if doDebug:
            ...
            self._debugger = None
            self.pushLine(
                'print "## Debug mode turned %s."' %
                (doDebug and 'on' or 'off'))
    ...
    def OnShellEnter(self, event):
        ...
        try:
            ...
            if cl == lc - 1:
                ...
                # Manage history
                if line.strip() and (
                        self.history and self.history[-1] != line or not self.history):
    ...
    def getCodeCompOptions(self, word, rootWord, matchWord, lnNo):
        if not rootWord:
            return list(self.interp.locals.keys()) + \
                list(__builtins__.keys()) + keyword.kwlist
        else:
            try:
    ...
    def getTipValue(self, word, lnNo):
        (name, argspec, tip) = wx.py.introspect.getCallTip(
            word, self.interp.locals)
...
EditorHelper.imgPyCrust = EditorHelper.addPluginImgs(
    r'Images\Editor\PyCrust.png')
