...
import Utils
from Explorers import ExplorerNodes, FileExplorer, scrm
...
class SVNController(ExplorerNodes.Controller):
    ...
    def doSvnCmd(self, cmd, svnDir, stdinput='', svnOutput='output window'):
        ...
        try:
            os.chdir(svnDir)
            SVNPD = ProcessProgressDlg.ProcessProgressDlg(
                self.list, cmd, 'SVN progress...')
            try:
...
class SVNFolderNode(ExplorerNodes.ExplorerNode):
    ...
    def __init__(self, entry, resourcepath, dirpos, parent):
        ...
        ExplorerNodes.ExplorerNode.__init__(
            self, name, resourcepath, None, svnFolderImgIdx, parent)
...
class SVNFileNode(ExplorerNodes.ExplorerNode):
    ...
    def __init__(self, entry, resourcepath, parent):
        ...
        if os.path.exists(filename):
            self.modified, self.conflict = svnFileLocallyModified(
                filename, entry)
            self.missing = False
        else:
        ...
        if self.conflict:
            ...
            if timestamp.startswith('Result of merge+'):
                ...
                if conflicts:
                    ...
                    editor.setStatus(
                        'No SVN conflicts in file', 'Warning', True)
