...
class FlatNotebookDTC(BookCtrlDTC):
    ...
    def designTimeDefaults(
            self, position=wx.DefaultPosition, size=wx.DefaultSize):
        defs = BookCtrlDTC.designTimeDefaults(
            self, position=wx.DefaultPosition, size=wx.DefaultSize)
    ...
    def writeImports(self):
        return '\n'.join((BookCtrlDTC.writeImports(
            self), 'import wx.lib.flatnotebook'))
...
Preferences.IS.registerImage(
    'Images/Palette/wx.lib.flatnotebook.FlatNotebook.png',
    getFlatNotebookData())
