...
class wxPythonDemoView(wx.Panel, EditorViews.EditorView,
    ...
    def __init__(self, parent, model):
        ...
        EditorViews.EditorView.__init__(
            self, model, actions=self.closingActionItems)
    ...
    def updateDemoCtrl(self):
        ...
        if demoModule:
            ...
            os.chdir(demoDir)
            try:
                self.demoCtrl = demoModule.runTest(
                    self.model.editor, self, self)
            finally:
                os.chdir(cwd)
...
class wxPythonDemoOverView(EditorViews.HTMLView, EditorViews.CloseableViewMix):
    ...
    def __init__(self, parent, model):
        EditorViews.CloseableViewMix.__init__(self)
        EditorViews.HTMLView.__init__(
            self, parent, model, actions=self.closingActionItems)
...
class wxPythonDemoSectionNode(ExplorerNodes.ExplorerNode):
    ...
    def openList(self):
        return [n for n in [self.createChildNode(
            name) for name in self.items] if n]
...
Preferences.IS.registerImage(
    'Images/Palette/wxPythonDemoModule.png',
    getwxPythonDemoModuleData())
