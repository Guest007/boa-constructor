...
class FileSysCatNode(ExplorerNodes.CategoryNode):
    ...
    def __init__(self, clipboard, config, parent, bookmarks):
        ...
        if not self.entries and wx.Platform == '__WXMSW__':
            ...
            self.updateConfig()
            wx.LogMessage(
                _('%d drives added to the filesystem definition.') %
                len(drives))
...
# return name
    ...
    def getNodeFromPath(self, respath, forceFolder=True):
        cn = self.createChildNode(
            os.path.basename(respath), respath, forceFolder)
        return cn
...
##            self.cvsController = None
        ...
        if 'cvs' in controllers:
            self.menu.AppendMenu(
                wxID_FSCVS, 'CVS', controllers['cvs'].fileCVSMenu)
        if 'svn' in controllers:
            self.menu.AppendMenu(
                wxID_FSSVN, 'SVN', controllers['svn'].fileSVNMenu)
    ...
    def OnFindFSItem(self, event):
        ...
        dlg = FindReplaceDlg.FindReplaceDlg(
            self.list, self.editor.finder, self.list, 0)
...
class FileSysNode(ExplorerNodes.ExplorerNode):
    ...
    def __init__(self, name, resourcepath, clipboard, imgIdx, parent=None,
        ...
        self.bookmarks = bookmarks
        self.exts = list(EditorHelper.extMap.keys()) + \
            list(EditorHelper.inspectableFilesReg.keys())
        self.entries = []
    ...
    def copyFileFrom(self, node):
        ...
        if not node.isDir():
            if node.resourcepath == os.path.join(self.resourcepath, node.name):
                newNameBase = os.path.join(
                    self.resourcepath, 'copy%s_of_' + node.name)
                num = ''
                while True:
...
##            shutil.copy(node.resourcepath, self.resourcepath)
            ...
            shutil.copytree(
                node.resourcepath, os.path.join(
                    self.resourcepath, node.name))
...
class ResultsFolderNode(FileSysNode):
    ...
    def getTitle(self):
        return _('Find results for %s in %s') % (
            self.lastSearch, self.resourcepath)
