...
class ZipController(ExplorerNodes.Controller,
...
class ZipItemNode(ExplorerNodes.ExplorerNode):
    ...
    def copyFromFS(self, fsNode, fn=''):
        ...
        if fsNode.isFolderish():
            ...
            # append new
            if new:
                zf = self.ArchiveClass(
                    self.zipFileNode.resourcepath, 'a', self.compression)
                try:
    ...
    def newFolder(self, name):
        ...
        try:
            ...
            zf.close()
        return name
        # raise Exception, 'The zipfile module does not suppport adding empty
        # folders'
    ...
    def replaceFilesInArchive(self, filesData):
        ...
        try:
            changed = False
            zfDst = self.ArchiveClass(
                self.zipFileNode.resourcepath, 'w', self.compression)
            try:
...
def findZipExplorerNode(category, respath, transports):
    zf = ZipFileNode(os.path.basename(category),
                     category, None, -1, None, None)
    zf.openList()
...
def isTarGzip(file):
    ...
    return ext == '.tgz' or ext == '.gz' and os.path.splitext(name)[
        1] == '.tar'
...
def findTarGzipExplorerNode(category, respath, transports):
    gzf = TarGzipFileNode(os.path.basename(category),
                          category, None, -1, None, None)
    gzf.openList()
